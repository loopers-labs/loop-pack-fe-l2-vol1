...
- **경계 = 변경 이유**: 한 컴포넌트가 서로 다른 이유로 바뀌면 자른다(분리 효과는 "수정 시 열어야 할 파일 수"로 본다). 비슷해 보인다고 성급히 합치면 `if(type)` 더미가 된다 — 중복 > 잘못된 추상화, 세 번째 중복에서 공통화(rule of three). 경계 4종 판단·Before/After는 docs "경계 — 어디서 자를까".
