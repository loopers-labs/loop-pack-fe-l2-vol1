...
- 동일한 도메인 값 목록(허용 카테고리·정렬 옵션 등)은 타입과 함께 한 곳(`_types` 등)에 배열로 정의하고 모든 사용처가 그걸 참조한다 — 같은 값을 여러 파일에 하드코딩 ✗. 새 검증/옵션 목록을 만들기 전에 같은 값이 이미 정의된 곳이 있는지 먼저 찾는다
- 배럴 파일(`index.ts`로 re-export) 지양 — 직접 경로로 import
- `src` 내부 import에는 `@/` 별칭 사용
