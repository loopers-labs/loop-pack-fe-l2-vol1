...
| R8  | 6주차 FSD       | 레이어 순서 `shared → entities → features → widgets → _pages → app`. 상위/동일 레이어 import, slice 내부 경로 deep import(Public API `index.ts` 우회) 금지. `src/app/api/**` 는 `_contract.ts` 만 사용 | `import ... from "@/features/auth/model/session-store"`   |
