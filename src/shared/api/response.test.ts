...
describe('apiResponseResult', () => {
  ...
  it('취소(AbortError)는 문구를 바꾸지 않고 그대로 다시 던진다', async () => {
    ...
    await expect(apiResponseResult('/api/products')).rejects.toThrow(abortError);
  });
  ...
  it('서버가 실패 응답에 메시지를 담아주면 그 메시지를 그대로 쓴다', async () => {
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue({
        ok: false,
        json: () => Promise.resolve({ message: '상품 목록을 불러오지 못했습니다.' }),
      }),
    );
  ...
  it('성공하면 응답 본문을 그대로 반환한다', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue({ ok: true, json: () => Promise.resolve({ totalCount: 30 }) }),
    );
