...
// app에서 처리하고, 하위 레이어에는 결과만 전달한다.

// 비로그인 상태와 세션 만료 상태를 구분한다. 각 상태에 필요한 안내가 다르기 때문이다.
...
export const readServerSession = async (): Promise<ServerSession> => {
  ...
  // 서버 렌더링에서도 시나리오 노브를 확인한다. API만 만료 응답을 반환하고 서버 렌더링은 로그인
  // 상태를 표시하면 헤더에는 이름이 있지만 목록 요청만 401이 되는 불일치가 발생한다.
 ...
 * proxy가 헤더로 전달할 수도 있지만, 그러면 경로가 요청 헤더를 통해 전달되는
