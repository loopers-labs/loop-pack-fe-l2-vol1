...
describe('scenario values', () => {
  it('accepts the four auth scenarios and nothing else', () => {
    expect(['invalid', 'expired', 'error', 'slow'].every(isAuthScenario)).toBe(
      true,
    );
...
describe('product id format', () => {
  it('accepts p1 through p30 and rejects anything outside that range', () => {
    expect(['p1', 'p9', 'p10', 'p29', 'p30'].every(isKnownProductId)).toBe(
      true,
    );
    expect(
      ['p0', 'p31', 'p999', 'p', 'p01', 'x1', ''].some(isKnownProductId),
    ).toBe(false);
  });
});
