...
export const accounts: Array<AuthUser> = Array.from(
  ...
  (_, index) => ({
    id: `u${String(index + 1)}`,
    name: `루퍼${String(index + 1)}`,
    email: `looper${String(index + 1)}@loopers.dev`,
  }),
)
...
const authScenarios = [
  ...
  'error',
  'slow',
] as const satisfies ReadonlyArray<AuthScenario>
...
const ordersByUser = new Map<string, Array<Order>>()
let orderSequence = 0

export const addOrder = (userId: string, items: Array<OrderItem>): Order => {
  orderSequence += 1

  const order: Order = {
    id: `o${String(orderSequence)}`,
...
export const listOrders = (userId: string): Array<Order> =>
