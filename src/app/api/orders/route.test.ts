...
describe('POST /api/orders', () => {
  ...
  it('rejects an empty item list, an unknown product and a non-positive quantity', async () => {
    ...
    expect(
      (
        await POST(
          postRequest({ items: [{ productId: 'p999', quantity: 1 }] }, cookies),
        )
      ).status,
    ).toBe(400)
    expect(
      (
        await POST(
          postRequest({ items: [{ productId: 'p1', quantity: 0 }] }, cookies),
        )
      ).status,
    ).toBe(400)
    expect(
      (
        await POST(
          postRequest({ items: [{ productId: 'p1', quantity: 1.5 }] }, cookies),
        )
      ).status,
    ).toBe(400)
...
describe('GET /api/orders', () => {
  ...
  it("returns only the signed-in user's orders", async () => {
    ...
    const first: unknown = await (
      await GET(getRequest({ [SESSION_COOKIE]: session(0) }))
    ).json()
    const second: unknown = await (
      await GET(getRequest({ [SESSION_COOKIE]: session(1) }))
    ).json()

    expect(first).toMatchObject({
      orders: [{ items: [{ productId: 'p1', quantity: 1 }] }],
    })
    expect(second).toMatchObject({
      orders: [{ items: [{ productId: 'p2', quantity: 3 }] }],
    })
  })
