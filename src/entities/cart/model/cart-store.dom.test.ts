...
it('담거나 빼면 저장소에 현재 목록과 버전을 기록한다', () => {
  ...
  expect(readStorage()).toEqual({
    state: { items: [{ productId: 'p2', quantity: 1 }] },
    version: STORAGE_VERSION,
  });
});

it('수량 변경을 현재 version 저장값에 반영한다', () => {
  cartActions().toggle('p1');
  cartActions().setQuantity('p1', 3);

  expect(readStorage()).toEqual({
    state: { items: [{ productId: 'p1', quantity: 3 }] },
    version: STORAGE_VERSION,
  });
});

it('변경할 상품이 없거나 값이 같으면 저장소를 다시 기록하지 않는다', () => {
  cartActions().toggle('p1');
  ...
  expect(savedItems()).toEqual([{ productId: 'p1', quantity: 1 }]);
...
it('되살린 뒤에도 담기와 빼기가 동작한다', async () => {
  saveCurrentVersion({ items: [{ productId: 'p1', quantity: 1 }] });
  await restore();

  cartActions().toggle('p2');
  cartActions().toggle('p1');

  expect(savedItems()).toEqual([{ productId: 'p2', quantity: 1 }]);
});

it('version 1 저장값은 버리지 않고 각 수량이 1인 항목으로 되살린다', async () => {
...
it.each([
  ...
  expect(savedItems()).toEqual([{ productId: 'p2', quantity: 2 }]);
});
...
it('같은 상품이 중복되면 앞의 항목만 남긴다', async () => {
  ...
  expect(savedItems()).toEqual([{ productId: 'p1', quantity: 2 }]);
});
