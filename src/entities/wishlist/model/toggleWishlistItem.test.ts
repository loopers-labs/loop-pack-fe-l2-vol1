 ...
 * `localStorage` 저장·복구는 `useWishlistStore.persist.dom.test.ts` 가 본다.
 *
 * 찜한 순서는 어디에도 노출되지 않으므로 단언하지 않는다.
 */

/** 이 테스트가 들고 있는 찜한 상품 목록. store 를 거치지 않는다. */
let wishlist: string[] = [];

/** 찜해 둔 상품 수. */
const wishedCount = () => wishlist.length;

/** 이 상품이 찜한 것인지. */
const isWished = (productId: string) => wishlist.includes(productId);

/** 사용자가 찜 버튼을 누른 것. */
const pressWishlist = (productId: string) => {
  wishlist = toggleWishlistItem(wishlist, productId);
};
...
describe('위시리스트', () => {
  ...
  // 경계 — 찜하지 않은 상품의 찜을 푸는 경우
  it('찜하지 않은 상품을 풀려 해도 찜한 상품은 그대로다', () => {
    pressWishlist('p1');
