...
### [9주차 — 인증과 E2E](../../tree/feat/week-09)

스타터 인증 API 위에 로그인·로그아웃·보호 경로를 붙이고, E2E를 많이 만드는 대신 **어디에 붙일지**를 30일치 시드 로그로 근거를 세워 정했습니다(인증 1 + 인증 외 2). 붙인 E2E가 실제로 값을 하는지 인증 구현을 한 곳씩 일부러 망가뜨려 확인했고, 리뷰 뒤 초록불 아래 결함 12건을 더 찾아 닫았습니다. → [E2E 범위 설계 문서](../../blob/feat/week-09/docs/rfc/week09-e2e-scope.md)

- **인증 경계** — 보호 경로는 주문서·주문 내역만. 진입 시 죽은 세션은 proxy가 쿠키의 서명·TTL을 검증해 막고, 진행 중 만료(보호 자원 401)는 queryClient 전역 한 곳이 맡습니다. 로그아웃해도 비회원 자산(장바구니·위시리스트)은 남기고, 복원 경로는 `safeRedirect`가 소비 시점에 다시 검증합니다(`//`·`/\`·이중 인코딩·traversal). 로그인 상태는 서버가 한 번 판독해 초기 HTML에 내립니다.
- **E2E 범위** — 세션 기준으로 집계하고 노이즈 세 종류(봇 486세션·중복 77줄·오류 103건)를 각각 다르게 걸렀습니다. 빈도 1·2위(목록·담기)엔 붙이지 않고 도달 하위권(3.7~8.6%)인 인증에 붙였습니다 — 실패 비용이 통합 테스트가 못 잡는 얽힘(proxy·쿠키·SSR)에 있어서입니다. 수치는 [재현 스크립트](../../blob/feat/week-09/docs/rfc/week09-seed-analysis.mjs)로 다시 만들 수 있습니다.
- **격리와 결정성** — 워커마다 다른 계정(looper1~8)과 `storageState`를 1회 만들어 재사용하되, 로그인 자체를 검증하는 테스트는 빈 상태에서 시작합니다. `--workers=4`와 `1`이 같은 결과를 냅니다.
- **자가검증** — 인증 세 곳을 한 곳씩 망가뜨려 E2E가 잡는지 봤습니다. proxy의 `matcher` 파손은 E2E만 잡았습니다(유닛은 함수만 불러 "어느 경로에 거나"를 건너뜁니다). 에이전트가 생성한 인증 테스트와도 비교해 걷어낸 검증 항목과 근거를 남겼습니다(생성물은 `docs/`에 증거로 보존).
- **리뷰 반영 — 초록불 아래 결함 12건** — 실제 만료 쿠키가 proxy를 통과하던 것, 프리페치된 307이 로그인 복원을 가로채던 것, `..//` 오픈 리다이렉트, 로그아웃 뒤 사용자 간 캐시 노출, 저장소 차단 시 앱 크래시 등을 재현(빨강)→고침(초록) 짝으로 닫았습니다. "살아남은 고장 없음"이 고른 세 건에 한한 결론이었음을 [RFC §F](../../blob/feat/week-09/docs/rfc/week09-e2e-scope.md#f-리팩토링--초록불-아래에서-찾은-것)에 기록했습니다. 유닛 236 → 263, E2E 25 → 29. ([커밋 기록](../../commits/feat/week-09))

### [10주차 — CI 파이프라인과 AI 협업](../../tree/feat/week-10)

검증을 더 만드는 대신, 있던 검증을 CI에서 재고·조건을 걸고·예산을 두고·AI 리뷰의 반복 지적을 결정적 룰로 내렸습니다. 측정과 실험은 fork에서, enforcement는 통합 브랜치 `kjeunn`의 required check로 뒀습니다. → [측정·게이트 기록](../../blob/feat/week-10/docs/rfc/week10-ci.md) · [10주 기술 회고](../../blob/feat/week-10/docs/rfc/week10-retrospective.md)
