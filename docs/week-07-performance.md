...
> **AI 협업 표기** — 초안 작성과 구현·측정은 AI가 맡고, 각 판단의 채택 여부와 방향은 직접 결정했다. 모든 판단의 근거를 측정값으로 남겼다. 어느 부분이 누구의 몫인지는 문서 끝 「AI 협업 표기」에 나눠 적었다.
...
| 상태 | 재현 방법 | 관찰 결과 |
| --- | --- | --- |
| COLD | 새 프로필로 첫 요청 | Route Handler 1회 호출(서버 로그로 계수) |
| WARM | 같은 request 안의 metadata + 본문 | **추가 호출 없음 — 계수 1회.** 응답 시간이 아니라 서버 측 계수로 확인했다 |
...
| 구분 | 범위 |
| --- | --- |
| **직접 결정 (최종 판단)** | **이 문서와 구현의 모든 결정을 최종 승인했다.** eslint 적용 범위를 어느 룰에 어떻게 둘지, Hero를 기존 `h1`을 유지한 채 연결할지, 커밋을 어떻게 나눌지, 슬라이드 자료를 레포에 넣지 않을지. 그리고 **어느 구간을 병목으로 볼지, 각 단계에서 어떤 개입을 고르고 무엇을 하지 않을지** — 아래 AI 산출물은 전부 검토를 거쳐 채택했다 |
| **AI가 초안 작성 (설계 판단), 검토 후 채택** | 구간별 병목 판정안(다운로드 92% → 발견 지연 78%), 개입 순서와 각 단계의 가설, `priority`를 새 개입이 아니라 Before 동작 유지로 보는 해석, TTFB +506ms를 감수하는 근거, 셸 스트리밍·`fetchPriority` 추가·서버 캐시·Advanced A를 제외하는 근거, 목록 6상태의 화면 구분 기준(`status`가 아니라 "보여줄 목록이 있는가"), metadata title·description 규칙. **측정값에 근거한 판단만 담았고 원본에 없던 개선은 얹지 않았다** |
