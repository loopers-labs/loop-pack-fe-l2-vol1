...
| `/` | 홈 `banner` 응답의 문구 (화면 `h1`과 원본이 다르다) | 홈 `banner` 응답 |
...
### 홈은 화면 문구와 metadata의 원본이 다르다

지금은 두 값이 같아 보이지만 **같은 출처에서 오는 것이 아니다.**

| | 출처 |
| --- | --- |
| 화면 `<h1>`과 설명 | `HomePage`가 소유한 정적 문자열 |
| `<title>`, `description`, `og:image` | `home.banner` 응답 |

fixture가 같은 문구를 담고 있어 일치할 뿐이다. **서버가 배너를 바꾸면 공유 카드만 바뀌고
화면은 그대로다.** 이미지는 이미 갈라져 있다. 화면 Hero는 `hero-original.jpg`이고
`og:image`는 `/images/products/p6.jpg`다.

**이건 LCP를 위해 의도한 분리다.** Hero 문구를 배너 응답에 묶으면 조회가 끝날 때까지 초기
HTML에 제목이 없다. 그것이 `3aa1981`의 LCP 42초를 만든 구조였고, `13b5113`에서 문구를 셸이
소유하게 바꾸면서 떼어냈다. 되돌리면 그 문제로 돌아간다.

목록의 카테고리 이름과는 성격이 다르다. 그쪽은 두 계약을 **동시에 지킨** 결과이고, 이쪽은
**렌더링 경계를 위해 원본을 나눈** 것이다. 배너 문구가 실제로 운영에서 바뀌는 값이 되면
화면과 공유 카드 중 어느 쪽을 정본으로 둘지 다시 정해야 한다.
