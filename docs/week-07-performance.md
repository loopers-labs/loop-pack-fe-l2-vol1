...
**"한글 서브셋 요청이 2개 64 KB에서 7개 183 KB로 늘었다"는 사실이 아니다.** Before, After,
라틴 치환 세 상태 모두 폰트 요청이 **10건 265,088 B로 같았다.** 따라서 요청 증가를 FCP 차이의
원인으로 설명할 수 없다. **같은 요청 집합이 만들어진 구체적인 이유는 이번 측정에서 확인하지
않았다.**

**"한글 문구가 원인 변수"도 성립하지 않는다.** 적용형 감속에서 한글 1,705 ms, 라틴 1,700 ms로
차이가 없다. 상관은 시뮬레이션에서만 나타난다.

폰트가 paint를 막는다는 설명도 두 가지 근거로 배제된다. Pretendard 서브셋 CSS의 `@font-face`
92개에 **모두 `font-display: swap`이 걸려 있고**, 적용형 감속에서 셸 문구에 필요한 서브셋
6개가 4,040 ms에 도착하는 동안 **FCP는 1,705 ms에 이미 일어난다.**
...
**감속 방식에 따라 다른 결과를 냈다는 것**이다. `simulate`는 감속 없이 관측한 뒤 Lantern이
사후 추정하고, `devtools`는 감속을 실제로 적용해 관측한다. **Lantern이 무엇을 다르게 계산해
899 ms가 나왔는지는 확인하지 못했다.**
...
원인을 찾아 나선 것이 처음의 잘못이었다. 요청 수를 잘못 세고, 그것을 원인처럼 배치했다.
지표가 나빠 보이면 **먼저 그 지표가 무엇을 재고 있는지부터 확인해야 했다.**
...
- **Lighthouse 기본 감속은 관측이 아니라 Lantern의 추정이다.** 홈 FCP의 899 ms 차이는 적용형
  감속에서 78 ms였고, Lantern이 무엇을 다르게 계산했는지는 확인하지 못했다.
