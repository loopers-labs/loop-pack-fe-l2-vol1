...
† TBT만 `36e31e0` 회차의 값이다. 나머지는 전부 `0785d2c` 회차다. `sizes`가 바뀐 뒤 TBT를
다시 재지 않았고, 후보 크기가 메인 스레드 작업량을 바꿀 이유가 없어 그대로 뒀다.
...
| 원본 대비 평균 절대차 | 3.12 | **1.46** |
| 원본 대비 최대 채널차 | 66 | **21** |

같은 페이지에서 최적화본과 원본을 같은 크기 캔버스에 그려 채널별 차이를 계산했다. 육안으로도
가방 가죽 결이 고치기 전에는 뭉개졌고 고친 뒤 되살아났다. 데스크톱은 원래 여유가 있어
변화가 없다(2.07 / 31 그대로).
...
사용자가 그 시점에 보는 것이 다르다. LCP 42.2초를 3.4초로 줄이고 CLS를 없앤 대가로
...
중복 제거 효과는 같고 native 쪽만 타임아웃을 잃는다. 응답이 오지 않는 upstream에 RSC 렌더가
매달리면, metadata를 기다리는 크롤러는 document 전체를 붙잡히게 된다. 얻는 것 없이 가용성을
내주는 교환이라 `cache()`를 택했다.
...
Lighthouse raw JSON 20개(약 14 MB)와 스크린샷은 **저장소에 커밋하지 않았다.** 크기 때문이고,
대신 같은 값을 다시 만들 수 있는 절차를 남긴다.
