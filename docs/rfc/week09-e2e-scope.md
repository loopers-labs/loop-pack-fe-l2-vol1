...
| 주문 플로우 | 세션 비율은 5.2%지만 구매 완료와 직접 연결된다. `order_start` 이후 `order_complete`로 가지 못한 세션도 33.9%라 실패 비용이 크다. | 로그인된 사용자가 장바구니에서 주문하기 → 주문서 진입 → 주문 완료 → 주문 내역 확인  |
...
주문 플로우는 인증 외에 선택한 E2E 경로다. 장바구니에서 주문 의도를 보인 뒤 실제 주문서에 들어가고, 주문 완료 후 주문 내역에서 결과를 확인하는 과정은 구매 전환의 마지막 단계다. store, 인증 쿠키, 주문 API, 라우팅이 모두 연결되므로 사용자 관점의 E2E로 한 번 확인한다.
