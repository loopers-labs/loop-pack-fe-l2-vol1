...
기준선의 로컬 evidence는 `$OMT_DIR/evidence/week06-fsd-migration/baseline/summary.md`와 `scenario-1.txt`~`scenario-8.txt`에 있다. 홈/목록 렌더, skeleton(홈 10·목록 12), 네트워크 오류의 인라인 오류와 재시도, 빈 결과, URL 공유(필터·정렬·페이지), 홈↔목록 장바구니/찜 동기화를 보존한다. 특히 `scenario-4.txt`는 현재 abort 시 필터·헤더를 유지한 인라인 오류와 재시도를, `unroute-check.txt`는 route 해제 뒤 정상 복구를 기록한다.

#### 구조 이동 재측정 — 2026-08-03, `e00aa5b`

구조 이동 재측정의 로컬 evidence는 `$OMT_DIR/evidence/week06-fsd-migration/structure-remeasure/`에 있으며 baseline과 재측정은 8/8 동일했다.
...
하향 의존은 `_app → widgets → features → entities → shared`와 `_pages → features/entities/shared`로 제한한다. 일반적인 슬라이스 밖 접근은 root `index.ts`의 named export만 쓴다. Entity 간 불가피한 관계만 producer의 소비자 전용 `@x/<consumer>.ts` 진입점을 예외로 쓴다. `export *`와 빈 폴더·미사용 barrel은 허용하지 않는다. `app/**`은 `depcruise src` 스캔 밖이므로, route의 slice import와 조립 테스트로 별도 관리한다.
...
feature store를 테스트·상위 조립에서 직접 reset해야 하는 경우만 `app/(commerce)/home-list-sync.test.tsx`가 `model/store`를 의도적으로 깊게 import한다. 이는 cross-slice를 조립하는 테스트의 한정된 예외이며, 실행 코드의 일반 public API 규칙과 Entity `@x/<consumer>.ts` 예외는 유지한다.
...
TDD는 `HttpError`와 guard, 두 page query의 `throwOnError` predicate, boundary의 두 reset 호출, home/list의 5xx·network 전파를 대상으로 했다. RED 로컬 evidence는 `$OMT_DIR/evidence/week06-fsd-migration/error-boundary/red.txt`(구현 전 7파일·10개 실패)이고, GREEN은 `$OMT_DIR/evidence/week06-fsd-migration/error-boundary/green.txt`(targeted 7파일·68테스트, 당시 전체 40파일·226테스트 통과)다. 최신 runtime HEAD `7f8463e` 브라우저 QA에서 boundary의 고정 안내 문구, 동일 URL의 새로고침 없는 복구, `window.__week06FinalMarker` 유지를 확인했다. 로컬 evidence는 `$OMT_DIR/evidence/week06-fsd-migration/final-qa/post-review-head.txt`다.
...
| RED → GREEN          | 전체 초기화 동작을 테스트로 추가                | 로컬 evidence `$OMT_DIR/evidence/week06-fsd-migration/advanced-b/{red,green}.txt`는 버튼 부재의 2개 RED 실패와 targeted 2파일·57테스트, 당시 `pnpm check` 40파일·228테스트 통과를 기록한다.                                                                                                                                                                                                    |
| 브라우저 URL/history | 비기본 상태는 한 번 push하고, 기본 상태는 no-op | 최신 HEAD `7f8463e` QA는 explicit-default URL(`/products?q=&category=all&sort=latest&page=1`) 전체 초기화의 no-op/history length 2 불변, unknown-only URL의 no-op/history 불변, 비기본 URL 초기화의 push 1회·back 복원을 PASS로 확인했다. invalid owned query는 계속 `/products`로 canonicalize했다. 로컬 evidence는 `$OMT_DIR/evidence/week06-fsd-migration/final-qa/post-review-head.txt`다. |
...
재현 명령은 `pnpm depcruise`와 `pnpm check`다. 최신 HEAD `7f8463e`에서 `pnpm check`는 PASS했고, 40파일·236테스트, 106 modules·266 dependencies·0 violations, build와 format 검사를 포함한다. 로컬 evidence는 `$OMT_DIR/evidence/week06-fsd-migration/final-qa/post-review-head.txt`다.

### 최종 브라우저 QA — 2026-08-03 기록

기록 당시 최종 QA는 전 항목 PASS였다. 비오류 scenario 1·2·3·5·6·7·8은 baseline과 7/7 동일했고, scenario 4는 오류 경계 정책과 무새로고침 복구를 확인했다. 최신 runtime HEAD `7f8463e`에서도 boundary 고정 안내 문구, 동일 URL의 새로고침 없는 복구와 marker 유지, explicit-default reset의 no-op/history length 2 불변, unknown-only reset의 no-op/history 불변, invalid owned query의 `/products` canonicalization, unsubmitted draft clear, 비기본 reset의 push/back 복원이 모두 PASS했다. 최신 실행 evidence는 `$OMT_DIR/evidence/week06-fsd-migration/final-qa/post-review-head.txt`다.

| 범위                    | 기록 당시 결과                                                                                                                                                                                                                                                     |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1. 홈 정상 렌더         | Hero·카테고리·인기/신상품과 ProductCard 12개가 baseline과 동일                                                                                                                                                                                                     |
| 2. 목록 정상 렌더       | 총 30개, ProductCard 12개, pagination과 기본 control이 baseline과 동일                                                                                                                                                                                             |
| 3. skeleton             | 홈 10개·목록 12개 skeleton이 baseline과 동일                                                                                                                                                                                                                       |
| 4. 오류 정책·복구       | network는 route boundary, 4xx는 control을 유지한 inline 오류였다. Header·marker·navigation entry 1을 보존했고, route 해제 뒤 boundary `다시 시도`와 fetch 복원 뒤 inline `재시도` 모두 같은 URL에서 새로고침 없이 복구했다.                                        |
| 5. 빈 검색              | 총 0개와 inline empty UI, URL/control 상태가 baseline과 동일                                                                                                                                                                                                       |
| 6. 필터 URL             | `q=케이스`, `category=digital`, `sort=price-asc`, ProductCard 2개가 baseline과 동일                                                                                                                                                                                |
| 7. pagination           | page 2 → back 1 → forward 2 → reload 2의 URL·첫 상품이 baseline과 동일                                                                                                                                                                                             |
| 8. cart/wishlist 동기화 | 홈 toggle 뒤 SPA 이동한 목록에서도 Header 1/1과 두 버튼 `aria-pressed=true`가 baseline과 동일                                                                                                                                                                      |
| 전체 초기화             | 비기본 URL은 push 1회 뒤 기본 URL/control/1페이지로 전환하고 back으로 시작 상태를 복원했다. explicit-default URL(`/products?q=&category=all&sort=latest&page=1`)과 기본 상태는 URL·history length 2를 바꾸지 않는 no-op이며 한 번의 back은 이전 페이지로 이동했다. |

### Architecture-review 수용/반려 — TODO 15 기록

architecture-review skill의 최종 tree 판정은 REQUEST_CHANGES, finding 3건이었다. review 로컬 evidence는 `$OMT_DIR/evidence/week06-fsd-migration/skill/review.txt`다. 아래 표가 각 지적의 수용/반려 판정 SSOT다.

| Finding                                                                                    | Review evidence                                                                                                                                                     | 수용/반려 | 근거                                                                                                                                                                                                                                                         | 후속 조치                                                                           |
| ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------- |
| `_pages` 테스트 3곳이 `app/api/**/route`를 직접 import하여 Pages → App 방향 의존을 만든다. | `review.txt`: `src/_pages/product-list/api/handler-parity.test.ts:3`, `src/_pages/product-list/ui/list-view.test.tsx:10`, `src/_pages/home/ui/home-view.test.tsx:7` | 반려      | 원본 플랜 line 20과 S-16은 기존 테스트 3곳의 상대 경로 재계산을 명시적으로 승인했다. handler parity와 오류 시나리오 검증을 위한 test-only edge이며, 런타임 `src` 그래프는 `pnpm depcruise`를 통과했다. depcruise의 `app/**` 범위 확장은 TODO 15 범위 밖이다. | 없음                                                                                |
| `app/(commerce)/home-list-sync.test.tsx`가 feature store를 deep import한다.                | `review.txt`: `app/(commerce)/home-list-sync.test.tsx:4-5`                                                                                                          | 반려      | 이 RFC의 기존 line 99와 S-16은 cross-slice assembly test에서 deterministic reset을 하기 위한 한정 예외로 사전 기록했다. store를 런타임 public API로 노출하면 오히려 은닉 계약이 약해진다.                                                                    | 없음                                                                                |
| `app/week-04/page.tsx`가 thin-route 규칙을 위반한다.                                       | `review.txt`: `app/week-04/page.tsx:2,48-66`                                                                                                                        | 반려      | Week 06의 목표는 `src/commerce` FSD migration이며 `src/products/**`는 strict-empty 보호 범위다. 이 legacy route는 이번 diff에서 생성하거나 악화한 코드가 아니다.                                                                                             | 별도 week-04 migration scope가 열릴 때 재검토한다. 이번 커밋에서는 변경하지 않는다. |

수용한 finding이 0건이므로 구조 코드 변경과 `pnpm check` 재실행은 필요하지 않았으며, 문서 형식 검사만 수행했다.
