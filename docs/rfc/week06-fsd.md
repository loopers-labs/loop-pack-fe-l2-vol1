...
## 2. API 에러·렌더링 에러 경계 설계 (과제 4단계)

### 처리 계층 (어디서 잡는가)

원칙: **가장 가까운·가장 국소적인 경계에서 잡는다.** 국소적으로 잡을수록 살아있는 화면이 넓다. 우리 앱의 계층:

- **공통 인터셉터** — `shared/api/requestJson`. 모든 요청의 응답을 한곳에서 가로채 `HttpError`(`status` 보유)·`NetworkError`로 변환한다(axios interceptor 자리). 우리 앱은 인증이 없어 401 리다이렉트 같은 공통 처리는 해당 없음.
- **전역 Error Boundary** — `app/error.tsx`(루트). catch-all이자, 홈의 `useSuspenseQuery` 에러가 도달하는 곳.
- **세그먼트(페이지)별 Error Boundary** — `app/products/error.tsx`. 목록만의 실패를 여기서 잡아 헤더·필터를 살린다.
- **컴포넌트별(경계 밖)** — `useQuery`의 빈 결과("검색 결과가 없습니다")·4xx(일반 오류 안내)를 목록 자리에서 처리(경계로 안 올림). 폼 필드별 인라인 검증과는 다르다.

`useSuspenseQuery`(홈)는 에러를 무조건 throw하므로 **Error Boundary가 필수**이고, 모든 실패가 경계로 간다(인라인 선택지가 없다). 반면 `useQuery`(목록)는 `throwOnError`로 **경계로 올릴 것(5xx·네트워크)과 경계 밖에서 국소 처리할 것(4xx)을 가를 수 있다.** 이 차이가 홈(전역, 헤더 덮음)과 목록(세그먼트, 헤더 유지)의 경험 차를 만든다.

### 실패 유형별 경계 설계표

| 실패 유형 | 처리 위치(계층) | Error Boundary로 전파하는가 | 사용자 UI | 재시도 방법 | 이 경계를 선택한 이유 |
| --- | --- | --- | --- | --- | --- |
| 상품 목록 조회 실패(5xx·네트워크) | **세그먼트 경계** · `app/products/error.tsx`(layout `안`) | **첫 로드는 O / background refetch는 X** — 볼 데이터가 없을 때만 `throwOnError`가 throw, 이미 목록을 보던 중 실패는 stale 유지 | 헤더·검색/정렬은 그대로, 목록 자리만 "상품 목록을 불러오지 못했습니다" + 다시 시도 버튼 | `resetQueries({ queryKey: productQueries.lists() })` + `reset()` — 전체 새로고침 없이 목록만 재요청 | 같은 5xx라도 목록만의 실패라 **전역이 아니라 세그먼트 경계**에서 잡아 헤더·필터를 살린다(부분 실패). ↔ **홈**의 같은 실패는 `useSuspenseQuery`라 루트(전역) 경계로 가 헤더까지 덮는다 — 세그먼트 경계가 없어 부분 실패가 안 됨(개선 대상) |
| 잘못된 검색 조건(4xx) | **컴포넌트별(경계 밖)** · `_pages/products` | X — `throwOnError`가 4xx는 전파 안 함 | 검색어 input 아래에 간단한 인라인 오류 문구(입력을 고치라는 검증이 아니라 상황 안내). 헤더·필터·목록 자리는 유지. *현재 전용 UI 미구현 → 후순위* | **없음** — 잘못된 검색 조건의 경우 같은 요청을 그대로 재시도하는 건 무의미. 잘못된 조건은 사용자가 만든 게 아니라 **FE가 명세를 따르지 않았거나 잘못된 옵션을 화면에 노출했거나 BE가 명세를 조용히 바꾼** 결과라 생각하여 복구는 코드·명세 수정 몫이지 사용자 재시도가 아니라고 판단. | 4xx는 사용자 입력 오류가 아니라 **FE가 잘못된 옵션을 노출했거나 BE가 명세를 바꾼 앱↔서버 계약 불일치**다. 발생 불가로 가정하진 않되, 사용자가 고칠 게 없으므로 검증 메시지·재시도 버튼이 아니라 검색어 input 아래 간단한 안내만 두고 복구는 코드·명세 수정에 맡긴다. |
| 예상하지 못한 렌더링 오류 | **전역 + 세그먼트** · `app/error.tsx`(루트) + `app/products/error.tsx` | O — 렌더 중 throw는 React가 가장 가까운 `error.tsx`로 보낸다 | 루트: "화면을 표시하지 못했습니다" + 다시 시도. 목록 세그먼트면 목록 자리만 교체 | 루트: `router.refresh()` + `reset()`을 `startTransition`으로 묶음(새 서버 트리로 커밋). 세그먼트: 위 조회 실패와 동일 | 데이터 오류든 렌더 버그든, 예측 못한 예외를 화이트스크린 대신 `error.tsx`의 복구 가능한 fallback으로 잡는다. |
| 장바구니 행위의 비즈니스 오류 | **컴포넌트별**(비즈니스 에러의 제자리) — 현재 **해당 없음** | X — 이벤트 핸들러라 애초에 경계가 못 잡음 | 해당 없음 | 해당 없음 | 담기·찜은 클라이언트 Zustand `Set` 토글이라 서버·검증·async가 없어 실패 경로 자체가 없는 상태. 게다가 이벤트 핸들러에서 나므로 Error Boundary로는 못 잡는다. 향후 서버 장바구니(재고 확인 → 품절 등)가 생기면 그 핸들러/뮤테이션 안(`try/catch` 또는 `useMutation` `onError`)에서 처리 — 아래 5 참조 |

### 실패 재현 결과

| 실패 유형 | 재현 결과 |
| --- | --- |
| 상품 목록 조회 실패(5xx·네트워크) | 목록이 있는 상태에서 조건을 바꿔 재요청이 5xx → `keepPreviousData`로 **이전 목록을 흐리게 유지**하고 경계로 안 감(볼 데이터 있으면 stale 유지). 볼 데이터 없는 실패는 세그먼트 경계 `app/products/error.tsx`로 가며, 화면은 아래 렌더 오류(목록)와 동일 — **헤더·검색/카테고리/정렬 유지, 목록 자리만 "상품 목록을 불러오지 못했습니다" + 다시 시도** |
