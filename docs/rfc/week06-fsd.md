...
architecture-review skill의 최종 tree 판정은 [REQUEST_CHANGES, finding 3건](/Users/toong/.omt/loop-pack-fe-l2-vol1/evidence/week06-fsd-migration/skill/review.txt)이었다. 아래 표가 각 지적의 수용/반려 판정 SSOT다.

| Finding                                                                                    | Review evidence                                                                                                                                                                                                                                            | 수용/반려 | 근거                                                                                                                                                                                                                                                         | 후속 조치                                                                           |
| ------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------- |
| `_pages` 테스트 3곳이 `app/api/**/route`를 직접 import하여 Pages → App 방향 의존을 만든다. | [review.txt](/Users/toong/.omt/loop-pack-fe-l2-vol1/evidence/week06-fsd-migration/skill/review.txt): `src/_pages/product-list/api/handler-parity.test.ts:3`, `src/_pages/product-list/ui/list-view.test.tsx:10`, `src/_pages/home/ui/home-view.test.tsx:7` | 반려      | 원본 플랜 line 20과 S-16은 기존 테스트 3곳의 상대 경로 재계산을 명시적으로 승인했다. handler parity와 오류 시나리오 검증을 위한 test-only edge이며, 런타임 `src` 그래프는 `pnpm depcruise`를 통과했다. depcruise의 `app/**` 범위 확장은 TODO 15 범위 밖이다. | 없음                                                                                |
| `app/(commerce)/home-list-sync.test.tsx`가 feature store를 deep import한다.                | [review.txt](/Users/toong/.omt/loop-pack-fe-l2-vol1/evidence/week06-fsd-migration/skill/review.txt): `app/(commerce)/home-list-sync.test.tsx:4-5`                                                                                                          | 반려      | 이 RFC의 기존 line 99와 S-16은 cross-slice assembly test에서 deterministic reset을 하기 위한 한정 예외로 사전 기록했다. store를 런타임 public API로 노출하면 오히려 은닉 계약이 약해진다.                                                                    | 없음                                                                                |
| `app/week-04/page.tsx`가 thin-route 규칙을 위반한다.                                       | [review.txt](/Users/toong/.omt/loop-pack-fe-l2-vol1/evidence/week06-fsd-migration/skill/review.txt): `app/week-04/page.tsx:2,48-66`                                                                                                                        | 반려      | Week 06의 목표는 `src/commerce` FSD migration이며 `src/products/**`는 strict-empty 보호 범위다. 이 legacy route는 이번 diff에서 생성하거나 악화한 코드가 아니다.                                                                                             | 별도 week-04 migration scope가 열릴 때 재검토한다. 이번 커밋에서는 변경하지 않는다. |
