...
| 브라우저 URL/history | 비기본 상태는 한 번 push하고, 기본 상태는 no-op | 최신 HEAD `973b0ee` QA는 unknown-only URL의 no-op/history 불변과 비기본 URL 초기화의 push 1회·back 복원을 PASS로 확인했다. 로컬 evidence는 `$OMT_DIR/evidence/week06-fsd-migration/final-qa/post-review-head.txt`다. |
...
재현 명령은 `pnpm depcruise`와 `pnpm check`다. 최신 HEAD `973b0ee`에서 `pnpm check`는 PASS했고, 40파일·235테스트, 106 modules·266 dependencies·0 violations, build와 format 검사를 포함한다. 로컬 evidence는 `$OMT_DIR/evidence/week06-fsd-migration/final-qa/post-review-head.txt`다.

### 최종 브라우저 QA — 2026-08-03 기록

기록 당시 최종 QA는 전 항목 PASS였다. 비오류 scenario 1·2·3·5·6·7·8은 baseline과 7/7 동일했고, scenario 4는 오류 경계 정책과 무새로고침 복구를 확인했다. 최신 runtime HEAD `973b0ee`에서도 boundary 고정 안내 문구, 동일 URL의 새로고침 없는 복구와 marker 유지, unknown-only reset의 no-op/history 불변, invalid owned query의 canonicalization, unsubmitted draft clear, 비기본 reset의 push/back 복원이 모두 PASS했다. 최신 실행 evidence는 `$OMT_DIR/evidence/week06-fsd-migration/final-qa/post-review-head.txt`다.
...
| 전체 초기화             | 비기본 URL은 push 1회 뒤 기본 URL/control/1페이지로 전환하고 back으로 시작 상태를 복원했다. 기본 상태는 URL·history를 바꾸지 않는 no-op이며 한 번의 back은 이전 페이지로 이동했다.                                          |
