...
> 상태: **전환 완료** · 2026-07-31 · 대상 브랜치 `volume-6`
>
> 이 문서는 FSD 구조 변경 **전에** 커밋됐다(`ad1b0cd` → 정정 `d220cbe`). 구조를 옮기기 전 기준선과 결정 근거를 먼저 고정하는 것이 목적이었다.
> 이후 전환(`cd2bf7e`)과 에러 경계(`ee288fb`)를 진행하며, 계획과 달랐던 지점은 문서를 고쳐 실제와 맞췄다 — 「진행 중 계획을 뒤집은 지점」 참조.
>
> **AI 협업 표기** — 초안 작성과 구현은 AI가 맡고, 각 결정의 채택 여부와 방향은 직접 판단했다. 어느 부분이 누구의 몫인지는 문서 끝 「AI 협업 표기」에 나눠 적었다.
...
각 단계마다 `pnpm check`를 돌리고 커밋을 나눈다. **각 커밋은 그 자체로 컴파일·테스트를 통과하는 상태여야 한다** — 파일만 옮기고 사용처를 다음 커밋으로 미루지 않는다.

| 단계 | 내용                                                                      | 커밋      | 규모  | 검증          |
| ---- | ------------------------------------------------------------------------- | --------- | ----- | ------------- |
| 1    | `shared` — `fetchJson`·`HttpError` 추출, `ui/{dialog,select}` 이동         | `095245f` | 9파일 | `pnpm check`  |
| 2    | `entities/product/model` — 통짜 `types.ts` 해체                            | `ac4b915` | 9파일 | `pnpm check`  |
| 3    | `entities/{cart,wishlist}` — store 2개로 분리                              | `e031d6e` | 6파일 | `pnpm check`  |
| 4    | `features` + `widgets` — `ProductCard` 표현/조합 분리 (문제 ② 해소)        | `c1e5db1` | 9파일 | `pnpm check`  |
| 5    | `_pages/product-list` — 목록 전용 로직을 페이지 슬라이스로                 | `8e70995` | 7파일 | `pnpm check`  |
| 6    | `_pages/home` — 화면 구현을 라우팅 밖으로                                  | `209bb2b` | 3파일 | `pnpm check`  |
| 7    | `_app` — provider·전역 스타일 분리, `app/`은 얇은 진입점                   | `8647282` | 5파일 | `pnpm check`  |
| 8    | Public API `index.ts` 도입 — 구체 경로를 슬라이스 계약으로                 | `34281a2` | 21파일 | **0단계 전체 재실행** |
| —    | 에러 경계 (**기능 변경 — 구조 커밋과 분리**)                                | `d4bd873` | 7파일 | #4 + 홈 error |

4단계가 features와 widgets을 함께 담는 이유: `ProductCard` 분리는 버튼(features)과 조합 지점(widget)이 동시에 있어야 컴파일된다. 둘로 쪼개면 중간 커밋이 깨진다.
8단계가 21파일인 이유: Public API 도입은 소비자 전부의 import 경로를 한 번에 바꾸는 작업이라 나눌 수 없다. 내용은 import 한 줄씩이다.
...
| ----------------------------- | -------------------------------- | ------------------------------------- | ---------------------------------------------- | -------------------------- | --------------------------------------------------------------------------------------------------------- |
| 상품 목록 조회 실패 (5xx·네트워크) | `app/products/error.tsx`     | **예**                                | 라우트 fallback + 다시 시도                    | `reset()`                  | 사용자가 조건을 바꿔도 해결되지 않는 실패다. 화면 안에 남겨두면 "다시 시도"만 반복하게 된다. 라우트 단위로 격리해 헤더는 살린다 |
| 잘못된 검색 조건 (4xx)        | `_pages/product-list` 인라인     | **아니오**                            | 목록 영역만 에러 문구 + 재시도 버튼            | `refetch()`                | 사용자가 URL로 만들 수 있는 상태다(`?page=0`, 0단계 #4). 필터 UI가 살아 있어야 조건을 바꿔 빠져나올 수 있다 |
| 홈 조회 실패                  | `_pages/home` 화면 단위          | **아니오**                            | 화면 전체 에러 + 재시도 버튼                   | `refetch()`                | ⚠️ **계획 수정** — 아래 메모 참조                                                                        |
| 예상하지 못한 렌더링 오류     | `app/products/error.tsx`         | **예**                                | 라우트 fallback + 다시 시도                    | `reset()`                  | 코드 버그라 화면 안에서 복구할 수단이 없다. 위 5xx와 같은 fallback을 공유한다                             |
| 장바구니 행위의 비즈니스 오류 | **해당 없음**                    | —                                     | —                                              | —                          | 현재 토글은 Zustand 로컬 연산이라 실패 경로가 없다. **서버 동기화나 재고 검증이 생기면** 필요 — 그때는 낙관적 업데이트 롤백 + 인라인 토스트 |

> **홈 항목 계획 수정 (구현 중 발견)** — 초안은 "배너·카테고리는 유지하고 상품 섹션만 에러"로 잡았으나 **구현 불가**였다.
> `/api/home`이 `banner + categories + popularProducts + newProducts`를 **한 응답으로** 준다. 조회가 실패하면 배너·카테고리를 그릴 데이터 자체가 없다.
> empty는 200 응답이라 배너·카테고리 데이터가 있어 섹션 단위 처리가 가능했던 것이고, error와는 조건이 다르다. 초안은 이 차이를 놓쳤다.
>
> 실제 적용: 화면 전체 에러는 유지하되 **재시도 버튼을 추가**해 전체 새로고침 없이 복구되게 했다(검증 C-2).
> 진짜 섹션 단위로 가려면 홈 엔드포인트를 쪼개거나(배너·카테고리 / 상품) 쿼리를 둘로 나눠야 한다 — 구조 변경 범위를 넘어서므로 다음 과제로 남긴다.
...
- [x] `shared/api/HttpError.ts` — `status` 보존
- [x] `fetchJson`이 `HttpError`를 던지도록 변경
- [x] `shared/api/isServerFault.ts` — 5xx·네트워크 판정을 한 곳에 모음
- [x] 목록에 `throwOnError: isServerFault` 적용
- [x] 목록: 에러 시 필터 UI 유지 + `refetch()` 버튼
- [x] 홈: 재시도 버튼 추가 (섹션 단위는 위 메모대로 불가)
- [x] `app/products/error.tsx` + `reset`
- [x] 임시 `throw` 없음 — 검증은 전부 응답 인터셉트로 했다(제품 코드 무변경)

**실제 검증 결과** (2026-07-31, dev 서버 + 응답 인터셉트)

| 케이스               | 재현                     | 결과                                                                        |
| -------------------- | ------------------------ | --------------------------------------------------------------------------- |
| 4xx 인라인           | `/products?page=0`       | `상품 목록을 불러오지 못했습니다. 검색 조건을 확인해 주세요. \| 다시 시도` · **필터 UI 생존** · `error.tsx`로 가지 않음 |
| 5xx 경계 전파        | `/api/products` → 500    | `error.tsx` 렌더 — `상품 화면에 문제가 생겼습니다` · 필터 사라짐 · **헤더는 생존**(라우트 단위 격리) |
| 홈 실패              | `/api/home` → 500        | `상품을 불러오지 못했습니다 \| 잠시 후 다시 시도해 주세요. \| 다시 시도` · 헤더 생존 |
| 홈 재시도 복구       | 인터셉트 해제 후 버튼 클릭 | hero `매일 새롭게 발견하는 취향` · 카드 12 복구 · **URL 그대로**(전체 새로고침 없음) |
...
> 폴더 이동이 **끝난 뒤** 코드를 수정하지 않고 답한다.
> ✅ **전환 완료 후 실제 구조와 대조 완료** (2026-07-31) — 예측과 실제가 일치했다. 대조 방법은 각 항목 끝에 적었다.
...
**판정**: 삭제 2폴더(4파일) + 수정 2파일, 모두 예측 가능. 수정 지점이 **조합 자리(widgets)에만** 생기는 것이 핵심이다.
전환 전이라면 `features/commerce/store.ts`를 열어 절반만 도려내고, `components/commerce/ProductCard.tsx` 안의 버튼과 훅 import를 찾아 지워야 했다 — **grep 없이는 못 찾는다.**

> **실제 대조**: `grep -rln "wishlist\|Wishlist" src/`에서 두 삭제 대상 폴더를 뺀 결과가
> `widgets/product-card/ui/ProductCardWithActions.tsx`, `widgets/header/ui/Header.tsx` **정확히 2개**였다. 예측과 일치.
...
> **실제 대조**: `createdAt`은 `entities/product/model/types.ts`에만 있고, 카드 렌더는 `entities/product/ui/ProductCard.tsx` 한 곳이다.
> 표현이 entity로 분리돼 있어 **홈·목록 어느 쪽도 건드리지 않는다** — 전환 전이라면 `components/commerce/ProductCard.tsx` 하나였겠지만, 그 파일이 store까지 알고 있어 뱃지 추가가 행위 로직과 같은 파일에서 일어났다.
...
`entities → features` 역방향 의존이다. FSD는 상위 레이어만 하위를 알 수 있는데, 찜 버튼은 features라 entities보다 위에 있다. 전환 전 코드가 정확히 이 상태였다 — `components/commerce/ProductCard.tsx`가 store를 직접 import해 버튼을 자체 렌더했다(61줄 중 18줄). 조합은 두 레이어를 모두 볼 수 있는 **widget**에서 한다. `entities/product/ui/ProductCard`는 `actions?: ReactNode` 슬롯만 받고, `widgets/product-card`가 버튼 둘을 끼워 넣는다.

**2. 한 페이지에서만 쓰는 검색 로직도 반드시 feature여야 하는가? 내 프로젝트에서는 어떻게 결정했는가?**

아니다. 초안에서는 `features/filter-products`로 뽑았다가 `_pages/product-list/{model,config,ui}`로 되돌렸다. FSD 2.1의 pages-first 기준으로 보면 검색·정렬 로직을 부르는 곳이 목록 1페이지뿐이라 Steiger의 `insignificant-slice`에 걸린다. "조건을 바꾼다"는 게 사용자 행위인 건 맞지만 그것만으로 feature가 되지는 않는다. 승격 신호는 **두 번째 페이지가 같은 것을 필요로 할 때**다.

**3. `formatPrice`는 항상 `shared/lib`인가? 통화·회원 등급·상품 정책이 포함되면 결정이 어떻게 달라지는가?**

아니다. 지금은 `Intl.NumberFormat("ko-KR")`로 숫자에 "원"만 붙이는 순수 포맷터라 `entities/product/ui/ProductCard` 안에 뒀다. 쓰는 곳이 한 군데뿐이라 `shared/lib`으로 내리면 그 자체가 speculative reuse다. 다만 **통화·회원 등급·할인 정책이 들어오는 순간 이건 포맷이 아니라 가격 정책**이 된다. 그때는 도메인 지식이 생기므로 `shared`가 아니라 `entities/product/lib`이 맞다 — `shared`는 도메인을 몰라야 하기 때문이다. 통화 코드만 다루는 순수 변환이면 그 부분만 `shared/lib`으로 남길 수 있다.

**4. 두 feature가 협력해야 할 때 직접 import하지 않고 어떤 상위 레이어에서 조합했는가?**

`widgets/product-card`에서 조합했다. `add-to-cart`와 `toggle-wishlist`는 서로를 모른 채 각자 버튼 하나만 내보내고, widget이 둘을 `actions` 슬롯에 나란히 넣는다. 같은 레이어 슬라이스끼리 직접 import하면 한쪽을 지울 때 다른 쪽이 깨진다. 실제로 삭제 시나리오를 돌려보니 위시리스트 제거 시 수정 지점이 조합 자리인 widget 2파일뿐이었다.

**5. 폴더 이동 후에도 TanStack Query 데이터와 Zustand 데이터를 서로 복사하지 않은 이유는 무엇인가?**

원본이 다르기 때문이다. 상품 정보의 원본은 서버이고 담긴 목록의 원본은 클라이언트다. 복사하면 두 값을 맞추는 규칙이 새로 필요해지고, 가격·재고가 바뀌는 순간 어긋난다. 그래서 store에는 **id만** 두고 상세는 필요할 때 Query로 조회한다. 헤더 개수도 저장하지 않고 `Object.keys(...).length`로 파생한다. 폴더가 바뀌어도 이 경계는 그대로여서, 기준선 #9·#10(페이지 이동 시 유지 / 새로고침 시 초기화)이 전환 후에도 같은 결과였다.

**6. barrel file과 Public API는 무엇이 다른가? 내 프로젝트에서는 어느 쪽을 선택했고 그 의도는 무엇인가?**

같은 `index.ts`라도 **무엇을 숨기려고 만들었는가**가 다르다. barrel은 경로를 줄이려는 습관적 재수출이라 경계 의도가 없고, `export *`가 쌓이면 이름 충돌·순환 의존·번들 비용만 남는다. Public API는 "외부가 알아도 되는 건 이것뿐"이라는 계약이다. 이 프로젝트는 **숨길 것이 있는 슬라이스에만** 뒀다 — `entities/cart`는 selector 3개만 공개하고 raw store를 숨긴다. 공개하면 외부가 store 전체를 구독해 리렌더 경계가 무너지기 때문이다. 반대로 `_pages/*`와 `shared/ui`에는 만들지 않았다. 소비자가 한 곳뿐이거나 숨길 대상이 없어 경로만 줄이는 barrel이 되기 때문이다. `export *`는 어디에도 쓰지 않았다.
...
| 구분                                    | 항목                                                                                                                                                                                                            |
| --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **직접 결정 (최종 판단)**               | **이 문서와 구현의 모든 결정을 최종 승인했다.** 전환을 진행할지·범위를 어디까지 할지, FSD 2.1 pages-first를 기준으로 삼을지, 5주차 목록 gcTime을 기본값으로 유지할지, 커밋을 어떻게 나눌지. 아래 AI 산출물은 전부 검토를 거쳐 채택했다 |
| **직접 결정 (5주차에서 이어짐)**        | 상태 소유권 표 원본(`week-05-design.md`), 캐시 정책 값과 근거 — 이번 주에 바꾸지 않기로 한 것도 결정이다                                                                                                        |
| **AI가 초안 작성 (설계 판단), 검토 후 채택** | A·D·I·O의 결정안 — 문제 4개 진단, 목표 트리, 레이어 선택과 미사용 결정, 파일 매핑표, 애매한 파일 결정표(6건), 상태 소유 슬라이스, Public API 방침, 에러 경계 표와 `throwOnError` 기준, 삭제 시나리오 예측     |
| **AI가 구현 (기계적), 검토 후 채택**    | 2~8단계 전환 코드 — 파일 이동·분할, `ProductCard` actions 슬롯화, cart·wishlist store 분리, `shared/api`(`fetchJson`·`HttpError`·`isServerFault`) 추출, `index.ts` 8개, `error.tsx`. **설계 결정을 코드로 옮긴 작업이고 새 설계를 얹지 않았다** |
| **AI가 실행·기록 (측정)**               | 0단계 기준선(상품 목록 10행 2026-07-29 · 홈 4상태 2026-07-31), 전환 후 기준선 10행 회귀 재실행, 에러 경계 4종 검증, 삭제 시나리오 실제 대조, `pnpm check`·`format:check` 결과                                  |
| **AI가 작성 (사실 추출)**               | 전환 전 폴더 트리, 내부 의존 그래프 10개 엣지 — 레포에서 그대로 뽑은 것                                                                                                                                         |

> 시간 제약으로 초안과 구현을 AI에 맡겼고, **각 결정의 채택 여부와 방향은 직접 판단했다.**
> 아래 셋은 트레이드오프가 갈려 근거를 밝혀 둔다.
>
> 1. **목록 쿼리·검색 필터를 페이지 슬라이스에 남긴 것** — FSD 2.1 pages-first를 기준으로 삼았다. 강의 자료가 entity-first(v2.0) 기준이면 배치는 달라질 수 있으나, 판단 근거는 "현재 부르는 곳이 목록 1페이지뿐"이라는 사실이다. 두 번째 페이지가 생기면 `entities`로 올린다.
> 2. **cart·wishlist store를 2개로 나눈 것** — 한 store 유지 + 슬라이스만 분리도 가능하다. 5단계 삭제 시나리오에서 폴더 삭제로 끝나는 쪽을 택했고, 실제 대조로 수정 지점이 widgets 2파일뿐임을 확인했다.
> 3. **`app/api/**`가 프론트 entity 타입을 참조하게 한 방향** — 실제 백엔드로 교체되면 사라질 의존이다. 반대 방향(프론트가 `app/api` 의존)은 라우팅 디렉터리에 도메인이 묶여 더 나쁘다고 판단했다.

### 진행 중 계획을 뒤집은 지점

설계가 구현에서 검증된(또는 반박된) 기록이다.

| 시점        | 무엇                                                              | 왜                                                                                             |
| ----------- | ----------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| RFC 정정    | 목록 쿼리·검색 필터를 `entities`·`features` → **페이지 슬라이스** | FSD 2.1 확인. 초안 근거였던 "재사용할 형태"가 곧 v2.1이 경고하는 speculative reuse였다        |
| 구현 중     | 홈 에러를 섹션 단위 → **화면 단위 + 재시도**                       | `/api/home`이 한 응답이라 실패 시 배너를 그릴 데이터가 없다. 초안이 empty와 error의 조건 차이를 놓쳤다 |
| 구조 커밋 전 | 구조 이동에 섞여 들어간 "다시 시도" 버튼을 **되돌림**              | 과제가 구조 변경과 기능 변경을 같은 커밋에 섞지 말라고 명시. 8단계 커밋으로 옮겼다             |
