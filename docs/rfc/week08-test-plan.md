...
## Advanced 실행 기록 — 9주차에서 반영 (2026-09-04)

8주차 리뷰: "Advanced를 범위만 정하고 실행하지 않았다. 손 실험 다섯 건 중 하나가 살아남았다는 것은 손으로 고르는 방식에 사각이 있다는 신호." 위 절의 범위 그대로 실행했다.

### 실행 조건

- `@stryker-mutator/core` · `vitest-runner` 10.0.0. 설정 `stryker.config.mjs`, 명령 `pnpm test:mutation`.
- 러너 설정은 `vitest.stryker.config.ts`로 분리 — **node 프로젝트(단위)만** 돈다. 본 설정의 `projects` 배열에서 하나만 고르는 옵션이 vitest-runner에 없다. 통합(jsdom+MSW)을 넣지 않는 이유는 위 절과 같다.
- pnpm은 node_modules를 격리하므로 core의 `@stryker-mutator/*` 자동 탐색이 러너를 못 찾는다 → `plugins`에 명시. 샌드박스(`.stryker-tmp`)는 eslint·tsconfig에서 제외(계측 사본이 `@ts-nocheck`라 666개 lint 에러를 낸다).
- **전체 실행 3~4초**(119 변형, 단위 24개 테스트). 상시 사용 가능한 비용이다.
...
| 파일 | 변형 | 왜 살았나 | 보강 |
| --- | --- | --- | --- |
| `wishlist/store.ts:9` | filter 콜백 → `() => undefined` / `item !== id` → `false` (빼기가 **전부** 지움) | 위시 테스트는 "p1 넣고 p1 빼면 []"만 봤다 — 전부 지워도 []이다. 장바구니에는 "가운데만 빼도 나머지 유지"가 있었고 위시에는 없었다 | 같은 경계 테스트를 위시에 추가 |
| `cart/store.ts:16` · `wishlist/store.ts:16` | 초기 상태 `[]` → `["Stryker was here"]` | "아무것도 담지 않았으면 비어 있다" 테스트가 실제로는 `beforeEach`의 `setState({cart: []})`를 보고 있었다. 이름과 달리 초기 상태를 검증한 적이 없다 | `getState()` → `getInitialState()` — 스토어가 선언한 값을 본다 |

두 번째 줄이 이번 실행에서 가장 값진 발견이다. 손 실험에서는 "초기 상태를 바꿔보자"는 생각 자체를 안 했다 — 리셋 코드가 있으니 검증됐다고 믿었다. 도구는 믿지 않는다.

**② 단위 범위에서 관측 불가 — 남긴다 (8개)**

| 파일 | 변형 | 이유 |
| --- | --- | --- |
| `cart` · `wishlist` 선택자 훅 6개 (`useCartCount` 등) → `() => undefined` | 훅은 React 렌더 안에서만 호출된다. 단위 테스트는 `getState()`로 스토어를 직접 본다 | 통합(담기 버튼 → 헤더 개수)이 잡는 영역. 단위에 넣으려면 `renderHook`(jsdom)이 필요해 범위 원칙과 어긋난다 |
| `products.queries.ts:9` `queryFn` → `() => undefined` | 단위 테스트(`productFilters.test.ts`)는 `queryKey`만 단언한다 — 그 테스트의 목적이 키 정규화다 | 통합(MSW 응답 → 목록 렌더)이 잡는 영역 |
| `products.queries.ts:10` `staleTime: 1000 * 60` → `1000 / 60` | 정책 상수. 어느 테스트도 단언하지 않는다 | 단위에서 `toBe(60_000)`을 쓰면 변경 감지 테스트가 된다. 사용자가 체감하는 건 "조건 변경 뒤 재요청 여부"라 그쪽에서 봐야 한다. 지금은 안 잡는 것을 알고 남긴다 |

**커버 없음 56개**: `useProductFilters.ts`(24, nuqs 훅)와 `shared/api/**`(26). 위 절에서 `shared/api/**`를 "단위로 분류한 파일"에 넣었지만 **실제로는 단위 테스트가 하나도 없다** — 전부 통합이 지나간다. 8주차 1단계 분류가 파일 기준으로는 맞았지만 테스트 존재 여부로는 틀렸다. 이번엔 분류만 정정하고 테스트는 늘리지 않는다.

### 손 실험 5건과의 비교

손 실험은 "내가 깨질 것 같다고 생각한 곳" 5곳이었고 1건이 살아남았다(`Math.max(1, …)`). 도구는 119곳을 바꿨고 그중 내가 몰랐던 사각을 4개 찾았다(초기 상태 2, 위시 경계 2). **손으로 고르면 자기가 아는 곳만 고른다**는 지적이 그대로 확인됐다. 다만 도구가 낸 12개 중 8개는 범위 밖이라 분류하는 눈은 여전히 사람 몫이다.

### 도구 결과를 손으로 검증하다 틀린 것

`item !== id → false` 변형을 처음엔 "삼항 조건 `list.includes(id)`를 false로"로 잘못 읽고 손으로 넣었더니 테스트가 실패해서 "도구 오탐"이라고 결론 내릴 뻔했다. 샌드박스의 계측 원문을 열어 보니 변형 위치는 filter **콜백 안**이었다. 도구 결과에 의심이 들면 리포트의 위치를 원문에서 확인한 뒤에 판단한다.

### E2E 진단성 (같은 리뷰의 세 번째 지적)

"요소 존재 대신 URL·상태를 먼저 단언" — 8주차 E2E 7개 모두 `goto` 직후 요소 단언으로 시작했다. 각각 `await expect(page).toHaveURL(...)`을 첫 단언으로 넣었다(`e2e/cart.spec.ts` · `e2e/filters.spec.ts`). 실패 시 "페이지가 안 열렸다"와 "요소가 안 그려졌다"가 갈린다. 9주차 E2E부터는 습관으로 가져간다.
