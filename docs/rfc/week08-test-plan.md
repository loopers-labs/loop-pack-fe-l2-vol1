...
**한 번 관측된 플레이크 — 원인 미확정.** 문서를 재검증하며 `pnpm test`를 다시 돌리던 중 한 번 `Test Files 1 failed | 11 passed`로 끝났다. 어느 파일이었는지 출력을 남기지 못했고, 이후 같은 명령을 23회(연속 8회·shuffle 6회·빌드 부하 병행 3회 포함) 돌렸으나 재현되지 않았다. 개별 테스트는 모두 통과였으므로 테스트 밖에서 난 오류일 가능성이 크고, 가장 의심스러운 자리는 **실패 경로 테스트가 남기는 재시도 요청**이다 — 재시도는 1초 뒤에 발사되는데 테스트는 그 전에 끝나므로, 남은 요청이 파일의 `afterAll`(`server.close()`) 이후에 도착하면 테스트는 전부 통과했는데 파일만 실패로 잡힐 수 있다. 확정하지 못했으므로 추측으로 고치지 않고 기록만 남긴다. 대응 후보는 `renderWithProviders`가 만든 QueryClient를 `afterEach`에서 `cancelQueries()`로 정리하는 것이다.
