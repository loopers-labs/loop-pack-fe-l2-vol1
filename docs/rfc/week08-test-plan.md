...
`persist` 왕복을 보는 파일(`*.persist.dom.test.ts`)은 여기에 더해 **자기 `beforeEach` 에서도 store 와 `localStorage` 를 비운다.** `setup.dom.ts` 의 `afterEach` 만으로도 다음 파일로는 새지 않지만, 이 파일은 저장소에 남은 것 자체가 검증 대상이라 시작 상태를 남에게 맡기지 않는다. node 쪽 파생 규칙 테스트는 store 를 아예 부르지 않으므로 초기화할 전역이 없다.
...
| --- | ----------------------------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| 1   | 장바구니·위시리스트 개수 파생             | **단위** | 개수는 store에 저장하지 않고 `cart.length` 로 파생한다. 파생 규칙은 `toggleCartItem` 순수 함수라 DOM 없이 검증된다. `persist` 저장·복구는 별개 관심사라 jsdom 파일로 갈랐다(부록). | 같은 상품을 두 번 담으면 헤더 개수가 2로 늘거나, 다시 눌러도 안 빠져서 **헤더 숫자와 실제 담긴 상품이 어긋난다**     |
...
리뷰 반영으로 항목 1이 두 조각으로 갈렸다 — **파생 규칙은 단위(node), `persist` 저장·복구는 통합(jsdom)**. 검증 항목 수는 그대로 15개이고, 갈린 근거는 부록에 적었다.
...
"URL을 건드리는가"를 기준으로 삼으면 8·9·10번도 전부 E2E로 가야 한다. 그 셋도 URL을 쓴다. 실제 기준은 그게 아니었고, **한 겹도 아니었다.** 층을 가르는 판정은 두 겹이다.

1. **무엇을 검증하는가** — 우리 코드의 규칙인가, 브라우저가 하는 일인가. 코드의 규칙이면 통합이다.
2. **그 층의 하네스가 그 검증을 재현할 수 있는가** — 재현하지 못하면 ①에서 통합이어도 올라간다.

| 항목   | ① 지키는 것        | URL이 견뎌야 하는 것      | ② 통합 하네스가 재현하는가                   | 층       |
| ------ | ------------------ | ------------------------- | -------------------------------------------- | -------- |
| 8·9·10 | 조건 → 목록 반응   | 없음 (같은 문서 안)       | 된다 — 단일 조작으로 충분                    | **통합** |
| **11** | 링크 공유 → 재진입 | **문서 경계를 넘는 왕복** | **안 된다** — 어댑터가 URL을 누적하지 않는다 | **E2E**  |
| 13     | 뒤로·앞으로        | history 스택              | 안 된다 — 어댑터가 스택을 쌓지 않는다        | **E2E**  |
| 14     | 새로고침           | 문서 재로드               | 안 된다 — jsdom 에 재로드가 없다             | **E2E**  |

**①의 기준은 "URL을 건드리는가"가 아니라 "URL이 문서 경계를 넘어 살아남아야 하는가"다.** 그 경계를 넘는 것만 통합이 흉내낼 수 없다. 11·13·14가 한 파일에 모인 이유이기도 하다.

그리고 **11번을 실제로 옮기게 만든 것은 ①이 아니라 ②였다.** ①만 보면 11번은 지금도 통합이다 — 지키려는 것이 브라우저 기능이 아니라 우리 코드의 결정이라는 처음 판단은 뒤집히지 않았다. 어댑터가 누적을 지원했다면 11번은 통합에 남았을 것이다. 두 겹을 갈라 적는 이유가 여기 있다. ①로 통합인데 ②로 올라간 항목은 **②가 사라지면 다시 내려야 한다.**

**되돌릴 조건**: `NuqsTestingAdapter` 가 URL 상태 누적을 지원하거나, 여러 조작이 쌓인 `searchParams` 를 관측할 다른 길이 생기면 11번은 통합으로 내린다. 13·14는 그때도 E2E에 남는다 — 그 둘은 ①에서 이미 브라우저 쪽이다.
...
**같은 자리에서 갈린 다른 선택**: 재호님·지원님은 11번을 통합에 남기고 `onUrlUpdate` 로 나가는 쿼리스트링을 직접 단언했다. 누적이 필요한 시나리오를 여러 조작으로 만들지 않고 **한 조작씩** 확인하는 방식이다. 갈림은 "어댑터 한계를 우회할 것인가, 층을 옮길 것인가"에서 생겼다.

|                 | 한 조작씩 단언 (통합 유지)                                                       | 왕복으로 확인 (E2E 이동)                                     |
| --------------- | -------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| 얻는 것         | 빠르다. 조작 하나가 URL에 실리는지 정확히 짚어 실패 지점이 좁다                  | 여러 조건이 쌓인 실제 링크를 그대로 검증한다                 |
| 포기하는 것     | **여러 조건이 쌓인 링크**는 끝내 못 본다 — 사용자가 실제로 보내는 것이 그 링크다 | 실행 시간(4.5s → 약 9s). 단언이 화면이라 실패 지점이 덜 좁다 |
| 무엇에 묶이는가 | nuqs 의 직렬화 규칙(기본값은 URL에서 지운다)에 단언이 묶인다                     | 실제 nuqs 어댑터의 동작에 묶인다 — 프로덕션이 쓰는 것과 같다 |

한 조작씩 보는 쪽이 틀렸다고 보지 않는다. 다만 내 경우 **옮기자마자 `?page=0` 버그가 잡혔고**(2.1), 그 버그는 조작이 쌓인 링크를 다시 여는 경로에서만 드러났다. 그 링크를 못 보는 배치였다면 이번 주에 찾지 못했을 것이다.
...
### 판단 3 — 캐시 키 검증(2번)을 상위 테스트가 있는데도 단위로 남긴 것

목록 조회는 E2E가 통째로 지나간다. 그런데도 `hashKey` 단위 테스트를 따로 둘 값이 있는가.

**두 테스트가 잡는 것이 다르다.** E2E가 확인하는 것은 "조건을 넣으면 그 결과가 보인다"이다. 캐시 식별 규칙이 깨졌을 때 나타나는 증상은 대부분 화면 오류가 아니다 — **같은 조건인데 요청이 두 번 나가거나, 다른 조건인데 남의 목록이 캐시에서 나온다.** 앞의 것은 E2E가 아예 보지 못하고, 뒤의 것은 타이밍이 맞아야 보인다. 화면이 옳게 보이는 동안에도 깨져 있을 수 있는 규칙이라 상위 테스트가 대신 서지 못한다.

**라이브러리를 검증한 것이 아니다.** `hashKey` 는 TanStack Query 의 구현이지만, 여기서 검증하는 대상은 `hashKey` 의 동작이 아니라 **내 코드가 만든 두 키가 같은 자리로 해석되는가**다. 라이브러리를 판정 도구로 쓴 것과 라이브러리를 검증한 것은 다르다. 해시 방식이 바뀌어도 이 테스트는 통과한다 — 두 키의 같음·다름은 그대로이기 때문이다.

**대체안을 고르지 않은 이유**: 실제 캐시 재사용을 보려면 같은 조건으로 두 번 렌더해 **MSW 가 받은 요청 수가 1인지**를 단언하는 통합 테스트가 된다. 사용자에 더 가깝지만 느리고 `staleTime`·재시도 설정에 민감하다. 깨졌을 때 "캐시 키가 갈렸다"인지 "설정이 바뀌었다"인지 구분되지 않아 진단 비용이 올라간다. 둘 다 두지도 않았다 — 같은 규칙을 두 층에서 잠그면 고칠 곳만 둘이 된다.
...
**2단계에서 만든 파일** — 15개 항목 전부 구현했다. 항목 1은 리뷰 반영으로 node·jsdom 두 파일로 갈렸다(아래).

| 파일                                                               | 항목           | 환경     | 테스트 |
| ------------------------------------------------------------------ | -------------- | -------- | ------ |
| `src/entities/cart/model/toggleCartItem.test.ts`                   | 1 (파생 규칙)  | node     | 8      |
| `src/entities/cart/model/useCartStore.persist.dom.test.ts`         | 1 (저장·복구)  | jsdom    | 4      |
| `src/entities/wishlist/model/toggleWishlistItem.test.ts`           | 1 (파생 규칙)  | node     | 8      |
| `src/entities/wishlist/model/useWishlistStore.persist.dom.test.ts` | 1 (저장·복구)  | jsdom    | 4      |
| `src/_pages/product-list/model/toProductListQuery.test.ts`         | 2              | node     | 16     |
| `src/features/product-filter/model/parseFilterParams.test.ts`      | 3              | node     | 19     |
| `src/_pages/product-list/ui/ProductListPage.dom.test.tsx`          | 4·5·6·7·8·9·10 | jsdom    | 17     |
| `src/widgets/header/ui/Header.dom.test.tsx`                        | 12             | jsdom    | 4      |
| `e2e/product-list-history.spec.ts`                                 | 11·13·14       | 브라우저 | 9      |
| `e2e/cart-flow.spec.ts`                                            | 15             | 브라우저 | 4      |

이 10파일만 세면 vitest 80 · Playwright 13 = **93 케이스**. (3단계 보강 2개, 리뷰 반영으로 갈라진 항목 1 포함)
여기에 0단계 스모크(jsdom 3 · E2E 2)와 기존 5파일 41을 더한 **전체 스위트가 vitest 14파일 124 · Playwright 15**이고,
`APP_ORIGIN=... pnpm check` exit 0 이다.

**항목 1을 두 파일로 나눈 이유**(리뷰 반영): 처음에는 `useCartStore.test.ts` 하나에서 store 를 `getState()` 로 직접 불러 개수 파생을 봤다. node 환경이라 `persist` 가 저장소를 찾지 못해 `set` 이 불릴 때마다 경고가 났다 — `[zustand persist middleware] Unable to update item 'commerce-cart', the given storage is currently unavailable.` (vitest 기본 리포터가 console 을 숨겨 `--reporter=verbose` 로 돌려야 보인다. 처음 이 경고를 "그냥 나는 것"으로 넘긴 데는 이 이유도 있다.)

경고의 원인은 **파일 배치가 아니라 store 가 `persist` 로 감싸여 있다는 것**이었다. 파일만 둘로 나눠도 node 쪽에서 store 를 부르는 한 경고는 그대로 난다. 그래서 넣고 빼는 순수 규칙을 `toggleCartItem` 으로 꺼내 node 에서는 그 규칙만 부르고, store 배선과 `persist` 왕복은 jsdom 파일에서 본다. 두 파일이 지키는 것이 실제로 다르다.

| 파일                                       | 무엇을 지키는가             | 깨지면 사용자가 겪는 일                |
| ------------------------------------------ | --------------------------- | -------------------------------------- |
| `toggleCartItem.test.ts` (node)            | 넣고 빼는 규칙 · 개수 파생  | 헤더 숫자와 실제 담긴 상품이 어긋난다  |
| `useCartStore.persist.dom.test.ts` (jsdom) | 저장소에 남고 다시 돌아온다 | 담아 둔 것이 다시 들어오면 사라져 있다 |

**값 치름**: `toggle` 이 슬라이스 안에서 모듈 export 가 됐다. Public API(`entities/cart/index.ts`)로는 여전히 내보내지 않으므로 외부와의 계약은 그대로다.
**얻은 것**: node 프로젝트 경고 0, 그리고 vitest 레벨에 없던 `persist` 검증이 생겼다 — 이전에는 저장·복구가 항목 15(E2E)에서만 지나갔다.

경고를 억누르지 않은 이유는 그것이 **"이 테스트가 자기 환경에 없는 것을 전제한다"는 신호**이기 때문이다. node 에 가짜 `localStorage` 를 주입해 지웠다면 같은 파일이 계속 두 관심사를 들고 있었을 것이다.

`persist` 테스트가 실제로 무언가를 잡는지는 3단계와 같은 방식으로 확인했다 — `partialize: () => ({})` 로 저장을 비우자 4개 중 3개가 실패했다(빈 저장소로 시작하는 경계 케이스만 통과). 저장이 멈추면 테스트가 붉어진다.
...
**하네스 스모크는 검증 항목과 섞지 않는다**(규칙으로 승격). `harness.dom.test.tsx` 와 `harness.spec.ts` 가 증명하는 것은 "jsdom 이 뜨고, 상대경로 요청이 MSW 에 잡히고, 어댑터가 앱 컴포넌트를 렌더한다"뿐이다. 15개 항목과 한 파일에 섞으면 통합 테스트가 깨졌을 때 하네스 문제인지 구현 문제인지 갈리지 않는다. 위 "남은 중복"의 비용은 이 갈림을 사는 값이다.

**helper 와 fixture 를 어디까지 묶는가** — 기준 셋으로 정리한다.

| 기준               | 내용                                                                                                  |
| ------------------ | ----------------------------------------------------------------------------------------------------- |
| 무엇을 감추는가    | **준비는 감춰도 되고 단언은 감추면 안 된다.** 테스트 본문만 읽고 무엇을 검증하는지 알 수 있어야 한다. |
| 언제 묶는가        | **세 번째 반복부터.** 두 번은 복붙이 더 읽기 좋고, 세 번째부터 고칠 곳이 세 곳이 된다.                |
| 언제 다시 쪼개는가 | **인자가 셋 이상 붙을 때.** 인자로 분기를 받기 시작한 helper 는 대개 두 개로 나눠야 할 신호다.        |

이 기준으로 지금 파일을 다시 읽으면 이렇게 갈린다.

| helper                                             | 성격 | 판정                                                   |
| -------------------------------------------------- | ---- | ------------------------------------------------------ |
| `pressAddToCart` · `pressWishlist` · `revisitWith` | 준비 | 감춰도 된다 — 사용자의 조작·재방문을 이름으로 부른 것  |
| `listHandler(all)`                                 | 준비 | 감춰도 된다 — 서버가 무엇을 돌려주는지의 배선          |
| `inCartCount()` · `isInCart()` · `productNames()`  | 단언 | 이름이 사실을 그대로 말한다. 그대로 둔다               |
| (쓰지 않은 예) `expectCartToBeValid()`             | 단언 | **금지.** 무엇이 "valid" 인지 테스트만 읽고 알 수 없다 |
