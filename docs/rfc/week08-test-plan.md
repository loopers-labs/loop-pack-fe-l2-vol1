...
**한 번 관측된 플레이크 — 원인 미확정, 예방 조치만 적용.** 문서를 재검증하며 `pnpm test`를 돌리던 중 한 번 `Test Files 1 failed | 11 passed`로 끝났다. 어느 파일이었는지 출력을 남기지 못했고, 이후 같은 명령을 33회(연속 18회·shuffle 6회·빌드 부하 병행 3회 포함) 돌렸으나 재현되지 않았다.

개별 테스트는 모두 통과였으므로 테스트 밖에서 난 오류로 보인다. 가장 의심스러운 자리는 **실패 경로 테스트가 남기는 재시도 요청**이다 — 재시도는 1초 뒤에 발사되는데 테스트는 그 전에 끝나므로, 남은 요청이 파일의 `afterAll`(`server.close()`) 이후에 도착하면 테스트는 전부 통과했는데 파일만 실패로 잡힐 수 있다.

이 가설을 프로브로 확인해 보려 했다. 실패 요청을 띄우고 재시도를 기다리지 않은 채 끝나는 테스트 파일을 만들어, 취소 로직을 켠 상태와 끈 상태로 각각 돌렸다. **둘 다 통과했다 — 재현에 실패했다.** 그래서 원인은 여전히 미확정이다.

그럼에도 `renderWithProviders`가 만든 QueryClient를 `afterEach`에서 `cancelQueries()`로 끊고 캐시를 비우도록 했다. 원인을 못 잡아서가 아니라, **테스트가 진행 중인 요청을 다음 테스트로 넘기지 않는다**는 격리 원칙 자체가 맞기 때문이다. 재현하지 못한 실패를 이걸로 고쳤다고 말할 수는 없으므로, 예방 조치로만 기록한다.
