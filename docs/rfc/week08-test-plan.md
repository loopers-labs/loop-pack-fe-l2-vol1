...
1단계에서 고른 15개 항목은 실제 테스트로 옮겼다. 단위 테스트는 DOM 없이 model과 query 계약을 확인하고, 통합 테스트는 Testing Library와 MSW로 사용자 조작부터 요청 조건과 화면 결과까지 본다. E2E는 통합 테스트가 이미 확인한 화면 로직을 반복하기보다, E2E를 제거했을 때 아무 테스트도 보지 못하는 production document 진입, 서버 Suspense fallback, 브라우저 history/reload, hydration 이후 연결만 보강한다.

테스트 이름과 하네스 규칙은 8주차 경험을 바탕으로 [`docs/ai/test-skill.md`](../ai/test-skill.md)에 따로 정리했다. 이번 주에 새로 작성한 테스트 이름은 국문으로 통일하고, 조건과 결과가 함께 드러나게 쓴다. 기존 스타터 테스트의 영문 이름은 이번 범위에서 일괄 변경하지 않지만, 새 파일과 새 케이스는 같은 규칙을 따른다.
...
| 카테고리 변경 → 목록 변경           | [`src/_pages/products/ui/ProductListPage.test.tsx`](../../src/_pages/products/ui/ProductListPage.test.tsx)                                                                                                                                                                                                                             | 없음                                                                                |
| 정렬 변경 → 순서 변경               | [`src/_pages/products/ui/ProductListPage.test.tsx`](../../src/_pages/products/ui/ProductListPage.test.tsx)                                                                                                                                                                                                                             | 없음                                                                                |
| 페이지 이동 → 목록 변경             | [`src/_pages/products/ui/ProductListPage.test.tsx`](../../src/_pages/products/ui/ProductListPage.test.tsx)                                                                                                                                                                                                                             | 없음                                                                                |
| 조작이 URL에 반영 · URL로 재진입    | [`src/_pages/products/ui/ProductListPage.test.tsx`](../../src/_pages/products/ui/ProductListPage.test.tsx)                                                                                                                                                                                                                             | [`e2e/products.spec.ts`](../../e2e/products.spec.ts): URL 직접 진입 복원            |
| 담기 → 헤더 개수 · 다시 누르면 빠짐 | [`src/_pages/products/ui/ProductListCommerceState.test.tsx`](../../src/_pages/products/ui/ProductListCommerceState.test.tsx)                                                                                                                                                                                                           | 없음                                                                                |
| 뒤로·앞으로 가기로 필터 복원        | [`e2e/products.spec.ts`](../../e2e/products.spec.ts)                                                                                                                                                                                                                                                                                   | 브라우저 history 경계                                                               |
