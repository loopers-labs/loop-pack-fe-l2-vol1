...
방법론마다 하나 이상. 눈에 잘 안 띄는 분기를 미리 고른다.

| # | 망가뜨릴 곳 | 어떻게 | 노림수 |
| --- | --- | --- | --- |
| 1 | `productFilterParsers`의 `parseAsStringLiteral(CATEGORIES)` → `parseAsString` | 정규화 제거 | 잘못된 category가 그대로 통과할 때 query key 단위 테스트(2번)가 잡는가 |
| 2 | `useProductFilters.setCategory`의 `page: 1` 삭제 | 호출 인자 하나 제거 | 8번의 경계 케이스(3페이지에서 카테고리 변경)가 실제로 존재하는가 |
| 3 | `SiteHeader`에서 `cartCount` ↔ `wishCount` 스왑 | 표시 값 교환 | 둘 다 0이면 안 잡힌다 — 실제로 담는 여정(15번)이 있어야 잡힌다 |
| 4 | `(commerce)/layout.tsx`에서 `<SiteHeader />` 삭제 | 호출 제거 | 12번을 통합에 둔 대가(레이아웃 조합은 못 잡는다)가 실제로 그런지 |

예상되는 생존자도 미리 적어 둔다: `ProductsResult`의 `Math.max(1, ...)`를 지우면 0건일 때 페이지 표시가 `1 / 0`이 되는데, 현재 5번 계획은 페이지 표시를 단언하지 않으므로 **살아남을 가능성이 높다.** 살아남으면 숨기지 않고 기록하고 5번에 단언을 추가한다.

실험은 구현 한 곳만 바꾸고 테스트 코드는 건드리지 않는다. 끝나면 되돌리고, 되돌린 상태로만 커밋한다.
