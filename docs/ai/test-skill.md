...
AI를 사용해 테스트 후보, 실패 원인 분석, 문서 초안, 코드 초안을 만들 수 있다. 다만 최종 제출물에는 어떤 판단을 사람이 직접 확정했는지 남긴다.

표기할 내용:

- AI가 도운 범위: 테스트 후보 정리, 실패 로그 해석, 문서 초안, 코드 초안 등
- 사람이 직접 정한 범위: 최종 설계 선택, 코드 반영, 검증 결과 확인
- 단언 내용: 어떤 사용자 관찰 결과를 테스트가 보장해야 하는지 직접 정했는가
- 모킹 경계: MSW, mock API server, 실제 API, `page.route()` 중 무엇을 쓰고 왜 선택했는가

테스트에서 가장 중요한 판단은 “무엇을 assert할지”와 “어디까지 mock할지”다. 이 두 가지를 AI가 제안했더라도 그대로 받아쓰지 않고, 과제 요구사항과 코드 구조에 맞는지 직접 확인한 뒤 확정한다.
