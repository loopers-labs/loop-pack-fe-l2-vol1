...
변경 범위를 감사하거나 OpenCode `/audit`의 범위, 에이전트, 심각도, 보고 형식을 확인할 때 읽는다.

## Source of truth

OpenCode 실행 지침은 `.opencode/commands/audit.md`와 `.opencode/agents/*.md`, 프로젝트 판단 기준은 `docs/rules/**`와 각 설정 파일이 우선한다.
...
- 감사가 요청 범위에 대해 read-only로 수행되었는지 확인한다.
- 설정 파일과 관련 leaf 문서를 근거로 사용했는지 확인한다.
- 실행하지 못한 검증과 수동 확인 필요 항목을 구분해 보고했는지 확인한다.
