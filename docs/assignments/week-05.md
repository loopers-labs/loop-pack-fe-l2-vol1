...
TanStack Query는 서버 데이터의 조회 상태와 캐시 수명을 맡습니다. `queryOptions`는 query key, queryFn, staleTime처럼 같은 조회를 설명하는 설정을 한곳에 모아 재사용하는 방식입니다. nuqs는 검색·카테고리·정렬·페이지처럼 링크로 공유해야 하는 조건을 타입이 있는 URL 상태로 다룹니다. Zustand는 서버나 URL에 원본이 없는 비로그인 사용자의 로컬 장바구니·위시리스트를 맡으며, 컴포넌트는 필요한 값과 action만 selector로 선택해 구독합니다.
