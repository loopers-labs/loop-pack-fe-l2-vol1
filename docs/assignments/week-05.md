...
상품 사진은 과제 저장소에 포함되어 있으며 앱 실행 중 29CM으로 요청하지 않습니다. 수집일, 원본 fixture commit, 상품 번호와 CDN URL은 `docs/assets/week-05-product-images.md`에 기록되어 있습니다. 출처 표시는 이미지 재배포 권리를 부여하지 않으므로 공개 배포 전 권리자의 허가를 별도로 확인해야 합니다.
...
- 상품명이나 이미지에서 확인할 수 있는 브랜드만 p11 `인스테드`, p16·p17·p19·p20 `스탠리`, p18 `렉슨`, p21 `메이커스`, p23 `위키오`, p25 `신지마운트`, p30 `신지루프`로 표시하며 나머지는 `29CM 셀렉트`를 사용합니다.
