...
- 상품 mock 데이터와 30개의 로컬 SVG 상품 일러스트 (`public/images/products/product-01.svg` ~ `product-30.svg`)
...
상품 일러스트는 이 과제를 위해 제작한 로컬 SVG 자산입니다. 앱 실행 중 외부 이미지 서버로 요청하지 않으며, 자산 목록은 `docs/assets/week-05-product-assets.md`에 기록합니다.
...
- p1은 `[11월 20일 예약배송] Winter Rocky Pants 2color 윈터 로키팬츠 OG`이며 가격은 79,000원입니다. 숫자 사이즈는 p1에만 있고, p2는 `freeShipping: false`와 빈 사이즈를 명시적으로 유지합니다.
- p11 `인스테드`, p16·p17·p19·p20 `스탠리`, p18 `렉슨`, p21 `메이커스`, p23 `위키오`, p25 `신지마운트`, p30 `신지루프`는 명시적 브랜드를 사용하고, 나머지 상품은 `Loopers Select`를 사용합니다.
