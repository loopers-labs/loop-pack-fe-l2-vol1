...
대상 화면은 `/`(`app/page.tsx`)이며, `ProductOptions`가 상품의 `optionKind`에 따라 렌더하는 3개 skin 표면(`src/products/skins/`) 각각을 self-baseline으로 비교한다.
...
3. `tsconfig.json`의 `exclude`에 `"e2e/**"` 추가 **(human 전용 — AI 편집 차단)** — 안 하면 tsc가 위 spec을 컴파일하려다 `Cannot find module '@playwright/test'`로 실패한다
4. `vitest.config.ts`의 `test.exclude`에 `"e2e/**"` 추가 — vitest가 `*.spec.ts`를 자신의 테스트로 오인해 실행하지 않도록 한다
5. 위 두 코드블록을 실제 `playwright.config.ts`·`e2e/skins-visual.spec.ts`로 저장한다
6. `.github/workflows/ci.yml`에 시각 회귀 job 추가 **(human 전용 — AI 편집 차단)** — `pnpm add -D @playwright/test` → `npx playwright install --with-deps chromium` → `pnpm build` → `npx playwright test`
7. `npx playwright test --update-snapshots`로 최초 baseline을 생성한다 — CI와 동일한 OS·브라우저 환경에서 잡아야 픽셀 차이가 나지 않는다
