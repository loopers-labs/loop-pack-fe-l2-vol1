...
| `products`, `totalCount`, `isLoading`, `error`                     | 서버 상태                    | `hooks/useProductListQuery.ts`       | 서버 상태 흐름은 UI 렌더링과 변경 이유가 다르다.                                     |
...
| `totalPages`, `pageNumbers`                                        | 파생값                       | `Pagination` 내부 계산               | 원본 값에서 계산 가능하므로 state로 저장하지 않는다.                                 |
...
`useProductListQuery`가 service 함수에 의존하도록 분리한다.
...
- `products`, `totalCount`, `isLoading`, `error` 상태와 요청 effect를
  `useProductListQuery`로 이동한다.
- `useProductListQuery`는 `getProducts` service 함수에만 의존한다.

Hook 한 문장 설명:

> `useProductListQuery`는 상품 목록 조회 조건에 따라 상품 목록 서버 상태를 가져온다.
...
- 페이지 번호 계산은 `Pagination` 컴포넌트 내부로 이동한다.
- `page` 자체는 URL query에서 온 값으로 유지한다.
- 페이지 변경 시 스크롤 이동은 외부 시스템인 브라우저 스크롤과의 동기화이므로
  effect 사용 근거가 있다.

분리 근거: 페이지 번호 목록은 `currentPage`, `pageSize`, `totalCount`에서 계산 가능한
파생값이므로 별도 Hook으로 만들지 않고 `Pagination` 내부 계산으로 유지한다.
