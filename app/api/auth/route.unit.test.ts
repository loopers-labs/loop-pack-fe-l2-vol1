...
import { accounts, TEST_PASSWORD } from '../_data/auth';
import { SCENARIO_COOKIE, SESSION_COOKIE } from '../_data/auth-cookies';
...
const sessionCookieFrom = (response: Response) => response.headers.getSetCookie().find((cookie) => cookie.startsWith(`${SESSION_COOKIE}=`)) ?? '';
...
describe('POST /api/auth/login', () => {
  it('returns the user and sets an httpOnly session cookie', async () => {
    const response = await login(loginRequest({ email: accounts[0].email, password: TEST_PASSWORD }));
  ...
  it('rejects a wrong password and an unknown email with 401', async () => {
    ...
    const unknownEmail = await login(loginRequest({ email: 'nobody@loopers.dev', password: TEST_PASSWORD }));
  ...
  it('rejects an unknown scenario before applying it', async () => {
    const response = await login(loginRequest({ email: accounts[0].email, password: TEST_PASSWORD }, '?scenario=empty'));
  ...
  it('fails with 401 in the invalid scenario even with correct credentials', async () => {
    const response = await login(loginRequest({ email: accounts[0].email, password: TEST_PASSWORD }, '?scenario=invalid'));
  ...
  it('fails with 500 in the error scenario', async () => {
    const response = await login(loginRequest({ email: accounts[0].email, password: TEST_PASSWORD }, '?scenario=error'));
...
describe('GET /api/auth/me', () => {
  ...
  it('returns 401 in the expired scenario from the query and from the cookie', async () => {
    ...
    const fromCookie = await me(meRequest({ [SESSION_COOKIE]: session, [SCENARIO_COOKIE]: 'expired' }));
    expect(fromCookie.status).toBe(401);
  });

  it('prefers the query scenario over the cookie scenario', async () => {
    const session = await signIn();
    const response = await me(meRequest({ [SESSION_COOKIE]: session, [SCENARIO_COOKIE]: 'expired' }, '?scenario=slow'));
