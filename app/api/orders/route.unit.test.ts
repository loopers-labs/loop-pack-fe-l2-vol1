...
import { accounts, createSessionToken, resetOrders } from '../_data/auth';
...
const getRequest = (cookies: Record<string, string>, query = '') => withCookies(new NextRequest(`http://localhost/api/orders${query}`), cookies);
...
describe('POST /api/orders', () => {
  ...
  it('rejects an empty item list, an unknown product and a non-positive quantity', async () => {
    ...
    expect((await POST(postRequest({ items: [{ productId: 'p999', quantity: 1 }] }, cookies))).status).toBe(400);
    expect((await POST(postRequest({ items: [{ productId: 'p1', quantity: 0 }] }, cookies))).status).toBe(400);
    expect((await POST(postRequest({ items: [{ productId: 'p1', quantity: 1.5 }] }, cookies))).status).toBe(400);
