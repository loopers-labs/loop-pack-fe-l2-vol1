...
import { addOrder, isAuthScenario, isKnownProductId, isRecord, listOrders, readSessionToken, waitForAuthApi } from '../_data/auth';
import { SCENARIO_COOKIE, SESSION_COOKIE } from '../_data/auth-cookies';
import type { AuthErrorResponse, AuthUser, OrderCreateResponse, OrderItem, OrderListResponse } from '../_data/auth';

type Resolved = { ok: true; user: AuthUser; scenario: string | null } | { ok: false; response: NextResponse<AuthErrorResponse> };

const resolveSession = async (request: NextRequest): Promise<Resolved> => {
  const scenario = request.nextUrl.searchParams.get('scenario') ?? request.cookies.get(SCENARIO_COOKIE)?.value ?? null;
  ...
  const user = scenario === 'expired' ? null : readSessionToken(request.cookies.get(SESSION_COOKIE)?.value);
...
export async function POST(request: NextRequest): Promise<NextResponse<OrderCreateResponse | AuthErrorResponse>> {
...
export async function GET(request: NextRequest): Promise<NextResponse<OrderListResponse | AuthErrorResponse>> {
  const resolved = await resolveSession(request);
  if (!resolved.ok) {
    return resolved.response;
  }
