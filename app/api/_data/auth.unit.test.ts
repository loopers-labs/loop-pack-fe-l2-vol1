...
import { accounts, addOrder, createSessionToken, findAccount, isAuthScenario, isKnownProductId, listOrders, readSessionToken, resetOrders, TEST_PASSWORD } from './auth';
...
describe('auth accounts', () => {
  it('provides eight accounts so parallel workers can take one each', () => {
    ...
    expect(accounts.map((account) => account.email)).toEqual(['looper1@loopers.dev', 'looper2@loopers.dev', 'looper3@loopers.dev', 'looper4@loopers.dev', 'looper5@loopers.dev', 'looper6@loopers.dev', 'looper7@loopers.dev', 'looper8@loopers.dev']);
