...
const authScenarios = ['invalid', 'expired', 'error', 'slow'] as const satisfies readonly AuthScenario[];

export const isAuthScenario = (value: string): value is AuthScenario => authScenarios.some((scenario) => scenario === value);
...
export const createSessionToken = (userId: string, nowMs = Date.now()) => {
  ...
  const payload = Buffer.from(JSON.stringify({ userId, iat: issuedAt, exp: issuedAt + SESSION_TTL_SECONDS })).toString('base64url');

  return `${payload}.${sign(payload)}`;
};

export const readSessionToken = (token: string | undefined, nowMs = Date.now()): AuthUser | null => {
  if (!token) {
    return null;
  }
