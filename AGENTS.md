...
특정 테스트만 실행하려면 `pnpm exec playwright test e2e/state-restoration.spec.ts`를 사용한다.
...
기본 검증은 `pnpm verify`를 사용한다. 단위·통합 테스트, 린트, 타입 검사를 실행하고 프로덕션 빌드는 하지 않는다.

```bash
pnpm verify   # pnpm test && pnpm lint && pnpm typecheck
```

`pnpm check`는 여기에 `pnpm build`를 더한 명령이라 사용자가 빌드를 요청했을 때만 실행한다.

변경 파일의 포맷이나 린트만 확인하면 되는 경우에는 해당 파일 범위에서만 정적 검사를 실행한다. `pnpm test:e2e`, `pnpm check`, `pnpm build`, `pnpm dev` 및 브라우저 기반 검증은 사용자가 요청한 경우에만 실행한다.

`pnpm verify`가 실행하는 Vitest 단위·통합 테스트는 브라우저를 띄우지 않으므로 기본 검증에 포함한다. 위 목록의 "Playwright를 포함한 테스트 실행"은 브라우저가 필요한 E2E를 가리킨다.
...
Week 10 과제를 질문·설계·구현·검토할 때는 작업을 시작하기 전에
`docs/assignments/week-10.md`와 `docs/week-10/checklist.md`를 함께 읽는다. 체크리스트의
필수·선택·과제 밖 범위와 에이전트 작업 전후 항목을 적용하고, 근거를 확인하지 않은 항목을 완료로
표시하지 않는다. required 검증, 예산 임계값, 비싼 검증의 실행 조건, AI 리뷰 배치와 승격할 규칙은
사용자가 직접 판단하도록 두며 에이전트가 대신 확정하지 않는다.
과제에서 가져온 요구사항을 설명할 때는 `docs/assignments/week-10.md`의 해당 섹션과 줄 위치를 함께
제시하고, 과제의 직접 요구·예시와 에이전트가 제안한 실행 방법을 구분한다.
Week 10 질의응답이 끝날 때는 새로 확인된 사실, 사용자가 내린 판단과 계획 변경을
`docs/week-10/action-plan.html`에 반영할 필요가 있는지 대조한다. 반영할 때는 기존 내용을 지우거나
덮어쓰지 않고 해당 단계 아래에 색이 다른 확인 이력으로 추가하며, 아직 확인하지 않은 내용은 완료로
표시하지 않는다.
