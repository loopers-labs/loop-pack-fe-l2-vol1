...
- Check that ESLint-enforced conventions match `docs/rules/15-Husky-품질-게이트.md`: array type notation, default export restrictions, direct-file imports with no slice/entity root barrel, the promoted-component-folder `index.ts` exception, Suspensive imports, `Show`/`For` rendering, and exported `lib` utility grouping.
