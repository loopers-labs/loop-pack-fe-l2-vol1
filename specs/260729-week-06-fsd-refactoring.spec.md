...
- [ ] 삭제 시나리오 2건(위시리스트 제거·신상품 뱃지)의 파일 목록 답변이 RFC에 있고 grep 대조와 일치한다 — 파일 목록 답변은 RFC의 위임 명시("PR 본문에 남긴다")에 따라 PR 본문에 기록
...
- [ ] (버그 발견 시) 구조 변경과 분리된 커밋 + `재현·원인·수정 위치·검증` 기록 — 해당 없음(이동 중 기존 동작의 버그 미발견)
