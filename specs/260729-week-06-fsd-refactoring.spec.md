...
- ESLint flat config (`eslint.config.mjs`) — `eslint-plugin-import-x` 사용 중, `import-x/no-cycle` 이미 켜짐. Advanced A는 기존 플러그인의 `no-restricted-paths`(zones)로 새 의존성 없이 가능. Steiger(FSD 공식 linter)는 새 의존성 (AGENTS.md: Ask first)
...
- D3: **Advanced A 하네스는 `import-x/no-restricted-paths` 단독** — 새 의존성 없이 기존 `pnpm lint` 게이트에 탑승. skill만으로는 Advanced A의 "자동 검증" 요건(결정적 게이트)을 충족하지 못하므로 하네스(기계 강제)와 skill(구조 판단 보조)을 별도 단계로 분리.
...
- T3: **Phase 2 — 하네스·skill 작성(D4)**: import-x zones 규칙 작성(위반 재현 증거 포함) + architecture-review SKILL 작성 — fulfills: 완료 조건 4, 9
