...
- gcTime은 별도로 늘리지 않고 TanStack Query 클라이언트 기본값 5분을 사용한다. 짧은 목록 재방문 캐시를
  보존하기에 충분하고, 사용하지 않는 조건별 목록 캐시를 더 오래 메모리에 둘 근거가 없기 때문이다.
- queryFn은 `response.ok`가 아니면 throw하고 성공 응답만 반환한다.
- 운영 환경에서 가격·재고 mutation이나 실시간성이 추가되면 staleTime과 무효화 정책을 다시 정한다.
...
- [ ] Q4. staleTime 1분과 기본 gcTime 5분의 선택 근거가 구현·PR 기록과 일치한다.
